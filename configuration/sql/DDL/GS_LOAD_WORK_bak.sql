CREATE OR REPLACE PACKAGE gs_load_work
AS
  /*
  version 1.0  08-feb-2018 g nelson
  version 1.2  09-feb-2018 g nelson
  versin 1.4  13-feb-2018  gn
  version 1.5  17-apr-2018  gn add in rcat
  version 1.6  03-may-2018  add user to email subject
  update gs_prices_RECEIVING_AREAS set GSP_PROCESSING_STATUS=null,GSP_PROCESSING_MESSAGE=null where GSP_BATCH_NAME='GRAHAMB1';
  */
...
begin
gs_load_work.load_prices();
end;
/

--select * from ft_t_isid ;

select * from MRU_GS_PRICES_RECEIVING_AREAS where GSP_BATCH_NAME = 'proc test 2'
;

update GS_PRICES_RECEIVING_AREAS set gsp_processing_status = null where rownum < 30
;

select * from GS_PRICES_RECEIVING_AREAS where gsp_processing_status is null
;

select * from GS_PRICES_RECEIVING_AREAS where gsp_processing_status is not null
;

select count(*) from GS_PRICES_RECEIVING_AREAS
*/